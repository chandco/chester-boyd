<?php
/**
 * Chester Boyd functions and definitions.
 *
...
function twentytwelve_setup() {
	/*
	 * Makes Chester Boyd available for translation.
	 *
	 * Translations can be added to the /languages/ directory.
	 * If you're building a theme based on Chester Boyd, use a find and replace
...
set_post_thumbnail_size( 450, 264, true );
add_image_size( 'single-post-thumbnail', 450, 264, 1 );
add_image_size( 'carousel', 1004, 412, 1 );

add_image_size( 'square-thumbnail', 150, 150 , 1);
...
if(!get_option("large_size_crop"))
    add_option("large_size_crop", "1");
else
    update_option("large_size_crop", "1");

add_image_size('old-blog-thumb', 430, 264, 1);
add_image_size('widescreen-large', 735, 412, 1);
...
function pw_show_image_sizes($sizes) {
    $sizes['pw-thumb'] = __( 'widescreen-large', 'pippin' );
    $sizes['pw-large'] = __( 'old-blog-thumb', 'pippin' );
...
// function to get the slug of a page
	function the_slug($echo=false, $permalink = false){
	if ($permalink)
	{	
...
function oikos_get_attachment_link_filter( $content, $post_id, $size, $permalink ) {
    ...
    if (! $permalink) {
        ...
        $image = wp_get_attachment_image_src( $post_id, 'widescreen-large' );
        $new_content = preg_replace('/href=\'(.*?)\'/', 'href=\'' . $image[0] . '\' data-lightbox=\'image-1\'', $content );
        return $new_content;
    } else {
        return $content;
    }
...
// the thing below will sort all custom post types archive to be listed in "order" rather than date.  So this might have to be overridden in a specific theme, but it's here to 'catch all' on post types like people / venues / food.
...
// if it's an archive page, and it's a custom post type, order by menu order rather than anything else...
add_filter('pre_get_posts', 'order_custom' );

function order_custom( $bargs )
{
	$types = get_post_types( array( "_builtin" => false ) );

	if ($bargs->is_archive and in_array($bargs->query_vars["post_type"],$types))
	{

		$bargs->query_vars["orderby"] = "menu_order";
		$bargs->query_vars["order"] = "ASC";
...
class recent_posts extends WP_Widget {
	function recent_posts() {
		parent::WP_Widget(false, 'Latest News');
	}
...
?>
	...
	function widget($args, $instance) {
		...
		$output = "<div class='widget-recent-posts' id='blog-panels'><h2>" . $instance["title"] . "</h2>\n";

		global $post;
		$args = array(
				'numberposts' => $instance['numPosts'],
				'post_type' => 'post',
...
register_widget('recent_posts');
	...
	function related_posts($args) {
		...
		$hotPosts = get_posts($args);
		if (count($hotPosts) == 0) return false;
		$output = "";
		foreach( $hotPosts as $post ) { 
